...
def get_args_parser():
    ...
    # 检测结果保存路径
    parser.add_argument('--output_dir', default='/home/zcs/code/deep-learning-for-image-processing-master/predict_demo/output/res_da',
                        help='path where to save, empty for no saving')
    # 存放模型的位置
    parser.add_argument('--resume', default='./weight_all/weight_res50_da/test/checkpoint_best_epoch64.pth',
...
parser = argparse.ArgumentParser('DETR training and evaluation script', parents=[get_args_parser()])
args = parser.parse_args()
schedule.every().day.at("18:00").do(lambda: main(args))  # 每天的10点运行脚本
