...
class SpatialAttention(nn.Module):
    def __init__(self, in_channels):
        ...
        self.conv = nn.Conv2d(in_channels, 1, kernel_size=7, stride=1, padding=3)
        self.sigmoid = nn.Sigmoid()
...
class TANet(nn.Module):
    ...
    def forward(self, x):
        ...
        out = out_position + out_channel + out_spatial
...
class SANet(nn.Module):
    def __init__(self, in_channels):
        ...
        # 另一个1x1卷积
        self.conv4 = nn.Conv2d(out_channels, in_channels, kernel_size=1, stride=1, padding=0)
    ...
    def forward(self, x):
        ...
        # 通过attention机制，使得每个通道的权重不同
        b, c, _, _ = x.size()
        w = self.avg_pool(y).view(b, c, 1, 1)
        w = self.sigmoid(w)
...
class ResNet_SA(nn.Module):
    ...
    def _forward_impl(self, x: Tensor) -> Tensor:
        ...
        x = self.layer3(x)
        # 在第三个卷积块之后添加SA-Net模块
        # 在ResNet50的第三个卷积块之后添加SA-Net模块的原因是因为在较浅的层次上使用注意力机制可以帮助模型更好地捕捉低级别的特征，同时也可以避免在较深的层次上出现梯度消失的问题。
        x = self.sa(x)
        x = self.layer4(x)
...
class ResNet_SE(nn.Module):
    def __init__(
        ...
        # 在最后一个 nn.Sequential 模块中添加 SEBlock
        # 我们在layer4的最后一个nn.Sequential模块中添加了SEBlock模块。
        # 改了resnet网络需要初始化senet部分的权重，还得加一部分代码
        for name, module in self.layer4.named_modules():
            if isinstance(module, nn.Sequential):
                if isinstance(module[-1], nn.Conv2d):
                    in_channels = module[-1].in_channels
                    if in_channels == 2048:
                        se_module = SEBlock(in_channels)
                        module.add_module('se', se_module)
...
class ResNet_DA1(nn.Module):
    def __init__(
        ...
        # 处理不同尺寸的输入，并且保持模型的通用性和可扩展性，可以采用自适应池化层
        self.avgpool = nn.AdaptiveAvgPool2d((1, 1))
        self.fc = nn.Linear(512 * block.expansion, num_classes)
...
def resnet50_se(num_classes=1000, pretrain_path=''):
    # https://download.pytorch.org/models/resnet50-19c8e357.pth

    model = ResNet_SE(Bottleneck, [3, 4, 6, 3], num_classes=num_classes)
    if pretrain_path != '':
        model.load_state_dict(torch.load(pretrain_path), strict=False)
    return model

def resnet50_da(num_classes=1000, pretrain_path=''):
    # https://download.pytorch.org/models/resnet50-19c8e357.pth

    model = ResNet_DA(Bottleneck, [3, 4, 6, 3], num_classes=num_classes)
    if pretrain_path != '':
        model.load_state_dict(torch.load(pretrain_path), strict=False)
    return model

def resnet50_sa(num_classes=1000, pretrain_path=''):
    # https://download.pytorch.org/models/resnet50-19c8e357.pth

    model = ResNet_SA(Bottleneck, [3, 4, 6, 3], num_classes=num_classes)
    if pretrain_path != '':
        model.load_state_dict(torch.load(pretrain_path), strict=False)
    return model
...
def resnet50_ta(num_classes=1000, pretrain_path=''):
    # https://download.pytorch.org/models/resnet50-19c8e357.pth

    model = ResNet_TA(Bottleneck, [3, 4, 6, 3], num_classes=num_classes)
    if pretrain_path != '':
        model.load_state_dict(torch.load(pretrain_path), strict=False)
    return model